...
from persistance.db_utils import get_slack_workspace_config_by, create_slack_bot_config, create_slack_workspace_config, \
    get_slack_bot_config_by_id, update_slack_bot_config
...
def handle_event_callback(data: Dict):
    if 'team_id' not in data or 'event' not in data:
        logger.error(
            f"Error while saving SlackBotConfig: team_id/event/authorizations not found in request data: {data}")
        return False
    team_id = data['team_id']
    event = data['event']
    active_slack_workspaces = get_slack_workspace_config_by(team_id=team_id, is_active=True, get_all_workspaces=True)
    if not active_slack_workspaces:
        logger.error(
            f"Error while saving SlackBotConfig in workspace {team_id}: active slack workspace not found")
        return False
    bot_user_ids = []
    for workspace in active_slack_workspaces:
        bot_user_ids.append(workspace.bot_user_id)
    if event and 'user' in event and 'type' in event:
        user = event['user']
        event_type = event['type']
        if user not in bot_user_ids:
            logger.error(
                f"Skipping event type: {event_type} handling for workspace {team_id}: {user} not found in bot users")
            return False
        channel_id = event.get('channel', None)
        event_ts = event.get('event_ts', None)
        if not channel_id or not event_ts:
            logger.error(
                f"Error while handling event type: {event_type} for workspace {team_id}: No channel_id/event_ts found")
            return False
                    ...
                    if is_created:
                        ...
                        if PUSH_TO_SLACK:
                            message_text = "Registered channel for workspace_id : " + team_id + " " + "channel_id: " + \
                                           channel_id + " " + "event_ts: " + event_ts
                            if channel_name:
                                message_text += " " + "channel_name: " + channel_name
                            publish_message_to_slack(message_text)
                    return True
                else:
                    logger.error(
                        f"Error while saving SlackBotConfig: {team_id}:{channel_id}:{event_ts}")
                    return False
            except Exception as e:
                logger.error(f"Error while registering slack with error: {e}")
                return False
            ...
            try:
                ...
                for bot_user_id in bot_user_ids:
                    if bot_user_id == user:
                        slack_workspace = active_slack_workspaces[bot_user_ids.index(bot_user_id)]
                        break
                slack_bot_config = get_slack_bot_config_by_id(slack_workspace.id, channel_id, is_active=True)
                if slack_bot_config:
                    updated_slack_bot_config = update_slack_bot_config(slack_workspace.id, channel_id, event_ts, False)

                    if updated_slack_bot_config:
                        if PUSH_TO_SLACK:
                            message_text = "De-Registered channel for workspace : " + slack_workspace.team_name + " " \
                                           + "channel_id: " + channel_id + " " + "event_ts: " + event_ts
                            if updated_slack_bot_config.channel_name:
                                message_text += " " + "channel_name: " + updated_slack_bot_config.channel_name
                            publish_message_to_slack(message_text)
                    return True
                else:
        ...
        logger.error(
            f"Error handling event in workspace {team_id}: No event found in request data: {data}")
        return False
