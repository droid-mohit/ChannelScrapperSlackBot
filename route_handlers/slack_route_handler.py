...
def handle_oauth_callback(data: Dict, account_id):
...
def handle_event_callback(data: Dict):
    ...
    active_slack_workspaces = get_slack_workspace_config_by(**active_slack_workspaces_filters)
        ...
        if event_type == 'member_joined_channel' or event_type == 'app_mention':
                ...
                if slack_bot_config:
                    if is_created:
                        ...
                        if PUSH_TO_SLACK:
                            ...
                            publish_message_to_slack(message_text)
                            data_fetch_job.delay(bot_auth_token, channel_id, str(event_ts), '')
                    return True
                else:
            ...
            try:
                ...
                if slack_bot_configs:
                    for slack_bot_config in slack_bot_configs:
                        updated_slack_bot_config = update_slack_bot_config(slack_bot_config.id, is_active=False)
            ...
            if 'api_app_id' in data and data['api_app_id'] == SLACK_APP_ID:
                for active_slack_workspace in active_slack_workspaces:
                    ...
                    for slack_bot in active_slack_bots:
                        updated_slack_bot_config = update_slack_bot_config(slack_bot.id, is_active=False)
                        if updated_slack_bot_config:
            ...
            try:
                for slack_workspace in active_slack_workspaces:
                    updated_slack_workspace = update_slack_workspace_config(slack_workspace.id, is_active=False)
                    if updated_slack_workspace:
                        ...
                        for slack_bot in active_slack_bots:
                            update_slack_bot_config(slack_bot.id, is_active=False)
                        if PUSH_TO_SLACK:
